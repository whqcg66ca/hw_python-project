...
while n <= NN: #50, 30, 60, 50 for canola, corn, soybean and wheat
#    knn = KNeighborsRegressor(n_neighbors=n)
#    knn = KNeighborsRegressor(n_neighbors=n,weights='distance')
    knn = KNeighborsRegressor( algorithm='ball_tree',n_neighbors=n, weights='uniform',metric='minkowski',leaf_size=10)
...
#fit_rf = RandomForestRegressor(n_estimators = n, random_state = 0,criterion = 'mse',max_features = 'auto') # two parameters of RF, number of tree, randomless
    knn.fit(x_train, y_train)  # fit the RF, x_train, y_train are fixed. no equator. This is a training process
    y_preds.append((knn.predict(x_test), n)) # predict the y_preds for different tree number. data.append is to combine the prediction without deleting the previous data
    sco = knn.score(x_test, y_test) # forest.score is to evalute the performance of the defined forest - forest with a tree number n
...
y_pred_final=knn.predict(x_test) # predict the phenology values using the determined random forest structure
